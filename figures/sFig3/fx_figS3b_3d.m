    ...
    if iDataSet == 1
        ...
        for iGroup = 1:numGroups
            ...
            for iCluster = 1:numClusters
                ...
                % rates for baselines
                if ~strcmpi(q1,badQ)
                    rates(1) = extract.cols(currentData,labels,measure,'cell num',clusterNums{iCluster},'exptHalf',expHalves{1},'session',sessions{1});
                end
                if ~strcmpi(q4,badQ)
                    rates(3) = extract.cols(currentData,labels,measure,'cell num',clusterNums{iCluster},'exptHalf',expHalves{2},'session',sessions{1});
                end
                ...
                if ~strcmpi(q2,badQ) && ~strcmpi(q3,badQ)
                    ...
                    if strcmpi(measure,'mean rate')
                        rates(2) = analyses.meanRate(sCNOday1,pCNOday1);
                    elseif strcmpi(measure,'mean field size (cm2)')
                        map = analyses.map(pCNOday1,sCNOday1,'smooth',hippoGlobe.smoothing,'binWidth',hippoGlobe.binWidth,'limits',hippoGlobe.mapLimits);
                        [~,fields] = analyses.placefield(map,'threshold',fieldThresh,'binWidth',hippoGlobe.binWidth,'minBins',hippoGlobe.minBins,'minPeak',minPeak);
                        if ~isempty(fields)
                ...
                if ~strcmpi(q5,badQ) && ~strcmpi(q6,badQ)
                    ...
                    if strcmpi(measure,'mean rate')
                        rates(4) = analyses.meanRate(sCNOday2,pCNOday2);
                    elseif strcmpi(measure,'mean field size (cm2)')
                ...
                for iComp = 1:size(compSesh,1)
                    dataStore(iCluster,iComp,iGroup) = (rates(compSesh(iComp,2)) - rates(compSesh(iComp,1))) / (rates(compSesh(iComp,2)) + rates(compSesh(iComp,1)));
                end
            end
        end
    ...
    elseif iDataSet == 2
        ...
        for iGroup = 1:numGroups
            ...
            for iCluster = 1:numClusters
                ...
                % rates for baselines
                if ~strcmpi(q1,badQ)
                    rates(1) = extract.cols(currentData,labels,measure,'cell num',clusterNums{iCluster},'session',sessions{1});
                end
                if ~strcmpi(q3,badQ)
                    rates(3) = extract.cols(currentData,labels,measure,'cell num',clusterNums{iCluster},'session',sessions{3});
                end
                ...
                if ~strcmpi(q2,badQ)
                    ...
                    elseif strcmpi(measure,'mean field size (cm2)')
                        map = analyses.map(pCNOday1,sCNOday1,'smooth',hippoGlobe.smoothing,'binWidth',hippoGlobe.binWidth,'limits',hippoGlobe.mapLimits);
                        [~,fields] = analyses.placefield(map.z,'threshold',fieldThresh,'binWidth',hippoGlobe.binWidth,'minBins',hippoGlobe.minBins,'minPeak',minPeak);
                        if ~isempty(fields)
                            fieldNum = length(fields);
                            peaks = nan(1,50);
                            for iField = 1:length(fields)
                ...
                for iComp = 1:size(compSesh,1)
                    dataStore(iCluster,iComp,iGroup) = calc.diffScore(rates(compSesh(iComp,1)),rates(compSesh(iComp,2)));
                end
            end
        end
    ...
    elseif iDataSet == 3
        ...
        for iGroup = 1:numGroups
            ...
            for iCluster = 1:numClusters
                ...
                if ~strcmpi(q4,badQ)
                    rates(3) = extract.cols(currentData,labels,measure,'cell num',clusterNums{iCluster},'session',sessions{4});
                end
...
text(-0.95,0.85,'p = 1.4 x 10^{-7}','fontsize',14,'fontweight','bold')
axis([-1 1 -1 1])

% size
axis([-0.2 1 -0.2 1])
text(-0.15,0.98,sprintf('r = %.2f',r),'fontsize',14,'fontweight','bold')
text(-0.15,0.85,'p = 6.1 x 10^{-6}','fontsize',14,'fontweight','bold')
