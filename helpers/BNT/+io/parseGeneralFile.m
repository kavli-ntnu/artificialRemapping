...
%
function trials = parseGeneralFile(filename)
    if strcmpi(filename, 'getReadMultipleValues')
        trials = localfunctions;
        trials = trials{1};
        return;
    end
    ...
    if isempty(strfind(lower(str), 'name'))
    ...
    headerNameInfo{1} = strtrim(headerNameInfo{1});
    headerNameInfo{2} = strtrim(headerNameInfo{2});

    if ~strcmpi(headerNameInfo{1}, 'name')
        error('BNT:inputFormat', 'Input file ''%s'' is of unknown format', filename);
    end

    if ~strcmpi(headerNameInfo{2}, 'general')
        error('Invalid type of the input file. Cannot read non-general input file with a function written for general files.');
    end
    ...
    exprKeywords = '^(sessions|cuts|units|shape|calibration|innersize|outersize|lfp|subject)';
    ...
    while ~feof(fid)
        ...
        if ~isempty(regexpi(str, '^sessions')) % match sessions word in the beginning of the str
            ...
            if numTrials > 1
                trials{numTrials}.inheritUnits = trials{numTrials-1}.inheritUnits;
            end
            if numTrials > 2
                if trials{numTrials-1}.inheritUnits
                    if isempty(trials{numTrials-1}.units)
                        trials{numTrials-1}.units = trials{numTrials-2}.units;
                    end
                end
            end
        ...
        elseif ~isempty(regexpi(str, '^units'))
            ...
            try
                ...
                doInherit = ~isempty(strfind(lower(unitsStr), 'inherit'));
                doDetection = ~isempty(strfind(lower(unitsStr), 'detect'));
                
                if doInherit
                    if numTrials > 1
                        trials{numTrials}.units = trials{numTrials-1}.units;
                    end
                    trials{numTrials}.inheritUnits = true;
                    continue;
                end
                if doDetection
                    % make it explicit in case mode has been changed
                    trials{numTrials}.inheritUnits = false;
                    continue;
                end
                ...
                for i = 1:length(allUnits)
                    ...
                    if curNumCells > 0
                        ...
                        units(startIdx, 1) = tmpUnits(1);
                        units(startIdx, 2) = nan;
                        numCells = numCells + 1;
                    end
                end
        ...
        elseif ~isempty(regexpi(str, '^subject'))
            trials{numTrials}.extraInfo.subjectId = strtrim(str(length('subject '):end));
        end
    end
