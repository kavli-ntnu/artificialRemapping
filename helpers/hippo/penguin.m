...
% Last Modified by GUIDE v2.5 18-Jul-2018 17:27:12
...
% --- Executes on button press in butt_gridPeaks.
function butt_gridPeaks_Callback(hObject, eventdata, handles)
% hObject    handle to butt_gridPeaks (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
...
while ishandle(f1)
    ...
    toKeep = inpolygon(x,y,[nodes(1),nodes(1)+nodes(3)],[nodes(2),nodes(2)+nodes(4)]);
    peak_count = peak_count + 1;
    peaks(peak_count) = max(map.z(toKeep));
...
colorMapBRK(map.z,'bar','on');
subplot(122)
bar(1:length(peaks),peaks,'facecolor',[.5 .5 .5])
axis square
box off
ylabel 'Peak rate (Hz)'
saveas(f2,fullfile(handles.userDir,sprintf('gridPeaks_T%sC%s.jpg',num2str(handles.tetrode),num2str(handles.cluster))));
